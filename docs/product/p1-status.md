...
| Flutter RoomWatch dogfood 闸门 | 完成 | 非 live 禁发聊；terminal 隐礼物栏；`FEATURE_PK` 软藏横幅；`H5_BASE_URL` / `shareRoomUrl` 分享链 |
...
| H5 RoomWatch 加载硬化 | 完成 | `loadSeq` 防竞态；`resetWatchRuntime`；`draftRoomId`；idle→live `onBecameLive`（含 Centrifugo）；Share 常显；失败态 UI |
